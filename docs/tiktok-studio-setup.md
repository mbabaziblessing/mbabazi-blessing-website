...
- Private Direct Post via PULL_FROM_URL; maximum 20 MiB (an application limit, not TikTok's maximum).
...
## 3. Create private temporary media storage

Create a private R2 bucket named `bless-content-studio-media`:

```powershell
npx wrangler r2 bucket create bless-content-studio-media
```

In the Pages project's **Production** bindings, add an **R2 bucket** binding named exactly `TIKTOK_MEDIA_BUCKET` and select `bless-content-studio-media`. Do not enable an `r2.dev` public URL. In the bucket's Object lifecycle settings, add a rule that deletes objects after one day; the app normally deletes each object sooner, but this rule is the cleanup backstop. Redeploy after adding the binding.

## 4. Configure the Pages environment
...
- PULL_FROM_URL is used. In the TikTok developer portal, verify ownership of the domain `www.mbabaziblessing.com` for Content Posting API media transfer. Complete the verification before testing; TikTok will reject an unverified media URL.
...
8. After an interrupted transfer, check Current session activity before creating a new upload. A session expiring ends access to its job history; check TikTok itself before reposting.
...
State expires after ten minutes. Sessions and jobs expire after at most one hour and cannot be used after expiry. Expired rows are physically removed on the next Studio request; they may remain in D1 until then. D1 backups and Cloudflare operational logs have their own retention settings. The implementation does not intentionally log credentials or private media URLs. Video bytes are temporarily stored in the private R2 bucket so TikTok can download them. The object is deleted after a final TikTok status, disconnection, or expiry cleanup; the one-day R2 lifecycle rule is a backstop. Video bytes are never stored in D1 or the public website. Metadata/captions are sent to TikTok, not retained as a content library.
...
- `503`: integration disabled, missing D1/R2 binding, missing secret/origin, or configuration not deployed.
...
- URL ownership error: verify `www.mbabaziblessing.com` in TikTok's Content Posting API settings and make sure the media URL is not redirected.
